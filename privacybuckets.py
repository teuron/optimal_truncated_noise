...
class PrivacyBuckets:
    ...
    def create_bucket_distribution(self, distr1, distr2, error_correction):
        ...
        if self.infty_bucket > _infty_bucket_warning_bound:
            self.logger.warning(
                "Infty bucket (numerical errors) is above {:g}. "
    ...
    def squaring(self):
        ...
        collapse = lambda arr: np.sum(arr.reshape(len(arr) // 2, 2), axis=1)  # noqa: E731
        ...
        if self.error_correction:
            ...
            div = lambda i: (1.0 / self.factor ** (2 * i - 1)) - (1.0 / self.factor ** (2 * i))  # noqa: E731
    ...
    def opt_compose_with(self, probability_buckets, after_squaring=False, threshold_factor=None):
        assert after_squaring is False and threshold_factor is None, "Function is being called in an unsupported way. We should fix this."
    ...
    def compose_with(self, pb, allow_modify_instance=False):
        ...
        if self.error_correction:
            ...
            convert_to_B = lambda distribution, factors: distribution / factors  # noqa: E731
    ...
    def compose(self, nr_of_compositions):
        ...
        get_state_filepath = lambda needed_exp: state_filepath_base + str(int(2 ** needed_exp))  # noqa: E731
    ...
    def delta_PDP(self, eps):
        ...
        return self.delta_PDP_upper_bound(eps)

    def delta_PDP_upper_bound(self, eps):
        """ Returns upper bound for tight delta for probabilistic differential privacy. Error correction not supported. """
        if self.error_correction:
    ...
    def delta_PDP_lower_bound(self, eps):
        """ Returns lower bound for tight delta for probabilistic differential privacy. Error correction not supported. """
        if self.error_correction:
            self.logger.warning("Error correction for PDP delta not supported. Omit error correction.")

        k = int(np.floor(eps / self.log_factor)) - 1
    ...
    def eps_ADP_upper_bound(self, delta):
        ...
        try:
            ...
            if self.delta_ADP_upper_bound(eps=max_eps) > delta:
                raise ValueError(
                    "Required target-delta is smaller than self.delta_ADP_upper_bound(max_eps) can serve. " "For an instant remedy, increase number_of_buckets, or increase factor."
                )
            elif self.delta_ADP_upper_bound(eps=0.0) < delta:
                self.logger.warning("Returning over-approximation eps=0. " "(self.delta_ADP_upper_bound(eps=0) < target-delta)")
                return 0.0
            raise e
    ...
    def renyi_divergence_upper_bound(self, alpha):
        ...
        assert self.distinguishing_events == 0 and self.infty_bucket == 0, "Nonzero infty bucket or distingushing events not supported"
...
class PrivacyBuckets_fromDelta(PrivacyBuckets):
    """
    Instructions besides the use of the parent PrivacyBuckets class (which can be found there) can be found at
    PrivacyBuckets_fromDelta.create_bucket_distribution
    """
    ...
    def __init__(self, delta_func, DP_type, **kwargs):
        ...
        super(PrivacyBuckets_fromDelta, self).__init__(**kwargs)
